...
```sh
./.venv-mac/bin/python sync.py transcripts --max-attempts 250 --delay 8
```

On Windows use `.\\.venv\\Scripts\\python.exe` instead. Progress and heartbeats
appear in the terminal. Each batch queries candidate Notion pages across active
playlist databases; it does not rescan YouTube metadata. Discovery can take several
minutes on large libraries. The attempt budget counts unique video fetches.
Requests wait at least the chosen delay plus jitter between attempts. The caption
...
Pending/Disabled/empty statuses are eligible; Error and Blocked respect the retry
cooldown. A block stops all further caption requests and persists a global cooldown
(at least one day, normally `transcript_retry_days`, seven days). Cached results can
still be saved during that cooldown. Individual transient/validation write failures
are deferred; rerun the same command to recover. Ctrl+C preserves cached results.
There is no automatic continuous loop: rerun batches as desired.
