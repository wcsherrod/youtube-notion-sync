...
### Local transcript backfill

After metadata import, run from the project directory on your local connection:

```sh
./.venv-mac/bin/python sync.py transcripts --max-attempts 250 --delay 45
```

On Windows use `.\\.venv\\Scripts\\python.exe` instead. Progress and heartbeats
appear in the terminal. Candidate snapshots are reused for 24 hours across restarts;
otherwise the batch queries candidate Notion pages across active
playlist databases; it does not rescan YouTube metadata. Discovery can take several
minutes on large libraries. The attempt budget counts requests, including retries.
Requests default to 45–56 seconds apart, increasing after blocks (up to a five-minute
base delay, or the requested delay if larger). The caption
library prefers manual captions over autogenerated captions in the configured languages.

The private `youtube-transcripts.sqlite` journal stores fetched captions before
Notion writes and reuses them across playlists and restarts. Keep this file to
resume failed saves. Full, Partial, and No captions returned pages are skipped.
Pending/Disabled/empty statuses are eligible; Error respects `transcript_retry_days`.
A block pauses caption requests for 30 minutes, then retries automatically. Repeated
blocks increase the cooldown to one, two, four, then six hours (the cap). These
cooldowns and slower pacing persist across restarts. Legacy seven-day global blocks
are shortened to at most 30 minutes on first launch of this version. The terminal
heartbeat stays active while waiting; Ctrl+C safely stops the wait. This cannot
guarantee YouTube will unblock the connection. Use `--no-wait-on-block` to skip
new requests during a block while still delivering cached results.
Individual transient/validation write failures
are deferred; rerun the same command to recover. Ctrl+C preserves cached results.
After the request budget or candidate list is exhausted, the batch exits; rerun
batches as desired. New candidate pages appear when the daily snapshot refreshes.

GitHub Actions now invokes `sync --transcripts off` to perform metadata updates
without caption extraction. Do not run local metadata sync and transcript backfill
concurrently; GitHub concurrency covers hosted runs only. Coordinate hosted runs
with local backfill, since both can update the same pages. Switching the hosted
transcript setting changes its checkpoint context once and starts a fresh scan.
Previously saved transcript content is retained.
